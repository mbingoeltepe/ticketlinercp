...
public class SearchTicketCommandHandler extends AbstractHandler implements
	IHandler {
    public static final String ID = "at.ticketline.kassa.ui.command.sucheTickets";
	...
	try {
	    if (!((new TicketSucheView()) instanceof INeedLogin)
		    || mitarbeiterService.isLoggedIn()) {
		page.showView(TicketSucheView.ID);
	    } else
		EasyMessage.showLoginRequiredMessage(ID);
	} catch (PartInitException e) {
	    MessageDialog.openError(window.getShell(), "Error",
		    "Suche Reservierte Tickets View konnte nicht geoeffnet werden: "
			    + e.getMessage());
	    this.log.error(e);
	}
	return null;
