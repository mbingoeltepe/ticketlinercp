 ...
 *
 */
public class NewsCommandHandler extends AbstractHandler implements
	IHandler {
    public static final String ID = "at.ticketline.kassa.ui.command.news";
	...
	try {
	    if (!((new NewsView()) instanceof INeedLogin)
		    || mitarbeiterService.isLoggedIn()) {
		page.showView(NewsView.ID);
	    } else
		EasyMessage.showLoginRequiredMessage(ID);
	} catch (PartInitException e) {
	    MessageDialog.openError(window.getShell(), "Error",
		    "News View konnte nicht geoeffnet werden: "
			    + e.getMessage());
	    this.log.error(e);
	}
	return null;
