 ...
 * 
 */
public class SearchVeranstaltungCommandHandler extends AbstractHandler implements
	IHandler, INeedLogin {
    public static final String ID = "at.ticketline.kassa.ui.command.sucheVeranstaltung";
	...
	try {
	    if (!((new VeranstaltungSucheView()) instanceof INeedLogin)
		    || mitarbeiterService.isLoggedIn()) {
		page.showView(VeranstaltungSucheView.ID);
	    } else
	    ...
	    MessageDialog.openError(
		    window.getShell(),
		    "Error",
		    "Suche Veranstaltung View konnte nicht geoeffnet werden: "
			    + e.getMessage());
	    this.log.error(e);
	}
	return null;
