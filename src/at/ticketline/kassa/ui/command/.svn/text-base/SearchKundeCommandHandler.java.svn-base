 ...
 * 
 */
public class SearchKundeCommandHandler extends AbstractHandler implements
	IHandler, INeedLogin {
    public static final String ID = "at.ticketline.kassa.ui.command.sucheKunde";
	...
	try {
	    if (!((new KundeSucheView()) instanceof INeedLogin)
		    || mitarbeiterService.isLoggedIn()) {
		page.showView(KundeSucheView.ID);
	    } else
	    ...
	    MessageDialog.openError(
		    window.getShell(),
		    "Error",
		    "Suche Kunde View konnte nicht geoeffnet werden: "
			    + e.getMessage());
	    this.log.error(e);
	}
	return null;
