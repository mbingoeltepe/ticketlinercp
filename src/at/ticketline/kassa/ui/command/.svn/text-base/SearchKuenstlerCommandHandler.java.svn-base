...
public class SearchKuenstlerCommandHandler extends AbstractHandler implements
	IHandler {
    public static final String ID = "at.ticketline.kassa.ui.command.sucheKuenstler";
	...
	try {
	    if (!((new KuenstlerSucheView()) instanceof INeedLogin)
		    || mitarbeiterService.isLoggedIn()) {
		page.showView(KuenstlerSucheView.ID);
	    } else
	    ...
	    MessageDialog.openError(
		    window.getShell(),
		    "Error",
		    "Suche Kuenstler View konnte nicht geoeffnet werden: "
			    + e.getMessage());
	    this.log.error(e);
	}
	return null;
